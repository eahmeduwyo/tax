...
window.addEventListener("DOMContentLoaded", () => {
  ...
  var loadStates = function () {
    ...
    statesLayer.queryFeatures(query)
      .then(function (response) {
        ...
        if (statesLoaded && medianLoaded && employmentLoaded) {
  ...
  // ACS Employment Status — State layer
  const employmentLayer = new FeatureLayer({
    url: "https://services.arcgis.com/P3ePLMYs2RVChkJx/ArcGIS/rest/services/ACS_Employment_Status_Boundaries/FeatureServer/0",
  ...
  const q = employmentLayer.createQuery();
  
  q.returnGeometry = false;
  q.outFields = ["STUSPS", "B23025_003E", "B23025_004E"];

  try {
    const res = await employmentLayer.queryFeatures(q);
    res.features.forEach(f => {
      const a = f.attributes;
      const abb = a.STUSPS;
      if (!abb || abb === "DC") return;

      const matchKey = Object.keys(stateObj).find(k =>
        stateObj[k] && stateObj[k].abb &&
        stateObj[k].abb.toUpperCase() === abb.toUpperCase()
      );
      if (!matchKey) return;

      const civLF = Number(a.B23025_003E) || 0; // civilian labor force
      const empl  = Number(a.B23025_004E) || 0; // employed in civilian Labpr Force
      const pct   = (civLF > 0) ? (100 * empl / civLF) : null;

      
      stateObj[matchKey].employmentPct = pct != null ? Number(pct.toFixed(2)) : null;
    });
    ...
    console.log("Employment data loaded");
  } catch (err) {
    console.error("loadEmploymentData failed:", err);
    employmentLoaded = true; 
    const submitBtn = document.querySelector('form button[type="submit"]');
    if (statesLoaded && medianLoaded && submitBtn) submitBtn.disabled = false;
  }
}
          ...
          if (value.notax == true) {
            $table.bootstrapTable('insertRow', {
              ...
              row: {
                ...
                employmentPct: value.employmentPct
              }
            })
            if (value.geometry) {
              let gfx = new Graphic({
